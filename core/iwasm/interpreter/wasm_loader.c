                    ...
                    uint32 error_buf_size)
{
    ...
    if (import_count) {
        ...
        for (i = 0; i < import_count; i++) {
		    ...
		    /* CHA: imported malloc func */
		    if (!strcmp(field_name, "malloc")) {
			    module->malloc_function = i; 
			    LOG_VERBOSE("Found malloc function, name: %s, index: %u", field_name, i);
		    }
                   ...
                   char *error_buf, uint32 error_buf_size)
{
    ...
    /* CHA: make malloc and free function -1 only when it is set to invalid value */
    int malloc_function_idx = 0;
    int free_function_idx = 0;

    if (module->malloc_function < module->import_function_count) 
	    malloc_function_idx = module->malloc_function;
    if (module->free_function < module->import_function_count) 
	    free_function_idx = module->free_function;
    /* CHA: finished */
    ...
    for (i = 0; i < module->export_count; i++, export ++) {
	    ...
	    /* CHA: set malloc and free function : to avoid error */
	    if (malloc_function_idx) module->malloc_function = malloc_function_idx;
	    //if (free_function_idx) module->free_function = free_function_idx;
	    /* CHA: finished */
        }
    }
